...
HTTPResponse_t requestJson(const TransportInterface_t *pTransportInterface,
                        const char *pMethod,
                        size_t methodLen,
                        const char *pHost,
                        size_t hostLen,
...
HTTPResponse_t requestJson(const TransportInterface_t *pTransportInterface,
  ...
  (void)HTTPClient_AddHeader(&requestHeaders, "Content-Type", sizeof("Content-Type") - 1U, "application/json", sizeof("application/json") - 1U);
...
void https_post() {
  ...
  // curl -XPOST -H 'Content-Type: application/json' https://httpbin.org/post -d '{"msg":"Hello, world!"}'
  HTTPResponse_t response = requestJson(&transportInterface, "POST", 4, hostname, 11, path, 5, body, bodyLen);
